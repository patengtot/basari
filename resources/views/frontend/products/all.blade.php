...
@section('title', 'Semua Produk — Basari')
...
<div class="flex items-center gap-3 mb-6">
    ...
    <nav class="text-sm text-gray-400">
        ...
        <span class="text-gray-600">Semua Produk</span>
    </nav>
</div>

<div class="mb-4">
    <h1 class="text-2xl font-bold text-gray-800">Semua Produk</h1>
    <p class="text-sm text-gray-400 mt-1">{{ $products->count() }} produk tersedia</p>
</div>
...
<div class="flex gap-2 flex-wrap mb-6 overflow-x-auto pb-1">
    ...
    <a href="{{ route('products.category', $cat->slug) }}"
       class="flex items-center gap-2 px-4 py-2 rounded-full border-2 text-sm font-medium transition flex-shrink-0
        ...
        @if($cat->image)
        <img src="{{ asset('storage/' . $cat->image) }}" alt="{{ $cat->localized_name }}"
...
<form method="GET" action="{{ route('products.all') }}"
    ...
    <div class="flex gap-2">
        ...
        <a href="{{ route('products.all') }}" class="btn-outline">Reset</a>
    </div>
...
<div class="mt-16">
    <div class="flex items-center gap-4 mb-8">
        ...
        <p class="text-xs uppercase tracking-[0.3em] text-gray-300">Stok Habis</p>
        <span class="flex-1 h-px bg-gray-100"></span>
    </div>
    <div class="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-10">
        @foreach($outOfStock as $product)
        <a href="{{ route('products.show', $product->slug) }}" class="group opacity-50">
            <div class="w-full aspect-[3/4] overflow-hidden bg-gray-50 mb-4 relative">
                <span class="absolute top-3 left-0 z-10 bg-gray-400 text-white text-xs uppercase tracking-[0.2em] px-3 py-1.5">
                    Habis
                </span>
                @if($product->images && count($product->images) > 0)
                <img src="{{ asset('storage/' . $product->images[$product->thumbnail_index ?? 0]) }}"
            ...
            <p class="text-xs uppercase tracking-widest text-gray-300 mb-1">{{ $product->category->localized_name }}</p>
            <p class="text-sm text-gray-500 font-light leading-snug mb-2 line-clamp-1">{{ $product->localized_name }}</p>
            <p class="text-sm text-gray-900 font-medium"
                data-price-idr="{{ $product->price }}"
                data-price-usd="{{ $product->price_usd ?? '' }}"
                data-price-myr="{{ $product->price_myr ?? '' }}">
...
<div class="text-center py-20">
    <p class="text-gray-400 mb-2">Tidak ada produk yang sesuai filter.</p>
    <a href="{{ route('products.all') }}" class="btn-primary inline-block mt-2">Reset Filter</a>
</div>
@endif
