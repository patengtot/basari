...
@endphp
    ...
    @endif
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-6">
            ...
            <p class="text-sm text-gray-400 mb-3">{{ __('app.contact_admin_desc') ?? 'Hubungi admin jika ada kendala dengan pesanan ini.' }}</p>
...
<div class="border-t border-gray-100 pt-3 mt-3 space-y-2">
    ...
    <div class="flex justify-between font-bold text-gray-800 text-base border-t border-gray-100 pt-2">
        ...
        <div class="text-right">
            ...
            @if($currency !== 'IDR')
            <p class="text-xs text-gray-400 font-normal">≈ Rp {{ number_format($order->total_amount, 0, ',', '.') }}</p>
   ...
   @if($order->courier)
    <div class="bg-white rounded-xl border border-gray-100 p-6">
        ...
        <div class="text-sm text-gray-600 space-y-2">
            ...
            @else
            <p class="text-xs text-gray-400 mt-2">{{ __('app.tracking_not_available') ?? 'Nomor resi belum tersedia.' }}</p>
            @endif
        </div>
    </div>
    @endif
...
<div class="bg-white rounded-xl border border-blue-100 p-6">
    <div class="flex items-center gap-2 mb-3">
        <span>🌍</span>
        <h2 class="font-semibold text-gray-800">{{ __('app.international_shipping') ?? 'Pengiriman Internasional' }}</h2>
    </div>
    <div class="text-sm text-gray-600 space-y-2">
        <div class="flex justify-between">
        ...
        <div class="bg-orange-50 border border-orange-200 rounded-lg px-4 py-3 mt-2">
            <p class="text-sm text-orange-700 font-medium">⏳ {{ __('app.waiting_shipping_cost') ?? 'Menunggu Konfirmasi Ongkir' }}</p>
            <p class="text-xs text-orange-600 mt-1">{{ __('app.waiting_shipping_desc') ?? 'Kami sedang menghitung ongkos kirim ke' }} {{ $order->destination_country }}.</p>
        </div>
        ...
        @else
        <p class="text-xs text-gray-400 mt-2">{{ __('app.tracking_after_shipped') ?? 'Nomor resi akan tersedia setelah paket dikirim.' }}</p>
        @endif
        @endif
    </div>
</div>
@endif
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-6">
        ...
        <div id="trackingLoading" class="text-center py-4">
            ...
            <span class="text-sm text-gray-400 ml-2">{{ __('app.loading_tracking') ?? 'Memuat status pengiriman...' }}</span>
        </div>
        ...
        <div id="trackingError" class="hidden text-sm text-gray-400 text-center py-4">
            ...
            <a href="https://track.biteship.com/{{ $order->biteship_tracking_id }}"
               target="_blank" class="text-blue-700 hover:underline">{{ __('app.track_at_biteship') ?? 'Lacak di Biteship' }} →</a>
        </div>
    </div>
    @endif
...
<div class="bg-white rounded-xl border border-gray-100 p-6">
    ...
    <p class="text-sm text-yellow-700 font-medium">{{ __('app.amount_to_pay') ?? 'Total yang harus dibayar:' }}</p>

    @if($order->shipping_type === 'international')
        {{-- Internasional: tampilkan dalam currency customer --}}
        <p class="text-2xl font-bold text-yellow-800">{{ $totalFmt }}</p>
        <p class="text-xs text-yellow-600 mt-1">≈ Rp {{ number_format($order->total_amount, 0, ',', '.') }} (IDR reference)</p>
    @else
        {{-- Domestik: tampilkan dalam IDR --}}
        <p class="text-2xl font-bold text-yellow-800">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</p>
    @endif
</div>
    ...
    {{-- Domestik: iPaymu --}}
    <p class="text-xs text-gray-400 mb-4">{{ __('app.payment_hint') ?? 'Klik tombol di bawah untuk melanjutkan ke halaman pembayaran.' }}</p>
...
@endif
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-6">
        ...
        <p class="text-sm text-gray-400 mb-4">{{ __('app.cancel_hint') ?? 'Pesanan masih bisa dibatalkan karena belum dilakukan pembayaran.' }}</p>
        <form method="POST" action="{{ route('orders.cancel', $order->id) }}"
              onsubmit="return confirm('{{ __("app.cancel_confirm") ?? "Yakin ingin membatalkan pesanan ini?" }}')">
            @csrf @method('PATCH')
            <button type="submit" class="bg-red-50 border border-red-200 text-red-600 hover:bg-red-100 font-medium py-2 px-4 rounded-lg transition text-sm">
                {{ __('app.cancel_order') }}
            </button>
        </form>
    </div>
    ...
    <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-5">
        <div class="flex items-start gap-3">
            ...
            <div>
                <p class="text-sm font-semibold text-yellow-700 mb-1">{{ __('app.cannot_cancel') ?? 'Pesanan tidak dapat dibatalkan' }}</p>
                <p class="text-xs text-yellow-600">{{ __('app.cannot_cancel_desc') ?? 'Pesanan ini sudah diproses dan tidak bisa dibatalkan.' }}</p>
            </div>
        </div>
    </div>
    @endif
            ...
            <div class="border border-gray-100 rounded-xl p-4">
                ...
                <div class="bg-gray-50 rounded-lg p-3">
                    ...
                    <div class="mt-3 bg-blue-50 border border-blue-100 rounded-lg p-3">
                        ...
                        <p class="text-xs text-blue-400 mt-1">{{ $existingReview->admin_replied_at->diffForHumans() }}</p>
                    </div>
                    @endif
                </div>
                @else
                    ...
                    <div class="mb-3">
                        ...
                        <div class="flex gap-1" id="stars-{{ $item->id }}">
                            ...
                            <label class="cursor-pointer">
                                ...
                                <svg class="w-7 h-7 text-gray-300 hover:text-yellow-400 transition star-icon" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                </svg>
                            </label>
                            @endfor
